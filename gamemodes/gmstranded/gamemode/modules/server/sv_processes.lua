...
function SGS_Mine( ply, rock, timemodi, modi, gem, void, autosmelt )
	...
	if ply:GetLevel("mining") < res.rlvl then
...
-- MINING --
------------
function SGS_Mine_Start(ply, res, len, modi, gem, void, autosmelt)
	if ply.inprocess == true then
		return
	end
	...
	timer.Create( ply:UniqueID() .. "processtimer", len, 1, function() SGS_Mine_Stop( ply, res, modi, gem, void, autosmelt ) end )

end

function SGS_Mine_Stop(ply, rock, modi, gem, void, autosmelt)
	if not ply:IsValid() then return end
	...
	if ( math.random(1,100) * modi ) * lvlmodi >= res.fail_chance then		
		/* check to see if we found a relic instead of a seed */
		local chance = 600
		if gem == "d" then chance = 500 end
		if ply.luck then
		...
		if size < 60 then
			if ( not ply:InGrindMode() ) and ( not autosmelt ) then
				ply:SendMessage("You managed to break free a small chunk of ".. string.lower(res.rname) .. "!", 60, Color(0, 255, 0, 255))
		...
		if size >= 60 and size < 90 then
			if ( not ply:InGrindMode() ) and ( not autosmelt ) then
				ply:SendMessage("You managed to break free a chunk of ".. string.lower(res.rname) .. "!", 60, Color(0, 255, 0, 255))
			elseif ply:InGrindMode() then
				ply:SendMessage("You are in grinding mode. Resources are not dropped.", 60, Color(0, 255, 0, 255))
			end
			xp = math.ceil( xp * 1.2 )
		...
		if size >= 90 then
			if ( not ply:InGrindMode() ) and ( not autosmelt ) then
				ply:SendMessage("You managed to break free a large chunk of ".. string.lower(res.rname) .. "!", 60, Color(0, 255, 0, 255))
			elseif ply:InGrindMode() then
				ply:SendMessage("You are in grinding mode. Resources are not dropped.", 60, Color(0, 255, 0, 255))
			end
			xp = math.ceil( xp * 1.5 )
		...
		if res.gem_chance and res.gem_chance > 0 then
			ply:CheckForGem(res.gem_chance)
			if gem == "d" then
				ply:CheckForGem(res.gem_chance)
				ply:CheckForGem(res.gem_chance)
				ply:CheckForGem(res.gem_chance)
